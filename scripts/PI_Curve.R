...
setwd("~/Documents/Projects/Thermal_Transplant_2017-2018/data/PI_curve") #set working

##### PHOTOSYNTHESIS Time 0 #####
path.p<-"../data/PI_curve/PI_curve_resp/" #the location of all your respirometry files 
...
PI.R <- data.frame(matrix(NA, nrow=length(file.names)*2, ncol=5))
colnames(PI.R) <- c("Fragment.ID","Intercept", "umol.L.sec","Temp.C","Light")

#Load Sample Info 
Sample.Info <- read.csv(file="Sample_info_PI.csv", header=T) #read in volume and sample.info data

#PHOTOSYNTHESIS
path.p<-"~/Documents/Projects/Thermal_Transplant_2017-2018/data/PI_curve/PI_curve_resp" #the location of all your respirometry files
...
for(i in 1:length(file.names)) { # for every file in list start at the first and run this following function
  ...
  PAR0 <- Photo.Data1 %>% filter(Time > '11:47:14' & Time < '12:09:00')
  
  #brk0 <- which(diff(Photo.Data1$Time) > 30) #look for breaks in time of 30 seconds or more
  #PAR0 <- subset(Photo.Data1, as.numeric(rownames(Photo.Data1)) < brk0)  #subset by break in time stamp keeping everything before break
  #REST0 <- subset(Photo.Data1, as.numeric(rownames(Photo.Data1)) > brk0)
  
  #brk1 <- which(diff(REST0$Time) > 30)
  #PAR1 <- subset(REST0, as.numeric(rownames(REST0)) < brk1)
  #REST1 <- subset(REST0, as.numeric(rownames(REST0)) > brk1)
  
  #subset by break in time stamp keeping everything before break
  lt.levs <- list(PAR0, PAR1) #list levels of segmentation
  ...
  for(j in 1:length(lt.levs)){    
    ...
    pdf(paste0("~/Documents/Projects/Thermal_Transplant_2017-2018/output",rename,"_",j,"thinning.pdf"))
    ...
    pdf(paste0("~/Documents/Projects/Thermal_Transplant_2017-2018/output",rename,"_",j,"regression.pdf"))
    plot(Regs)
    dev.off()
...
#Load Sample Info
Sample.Info <- read.csv(file="Colony_Sample_Info_T1.csv", header=T) #read in volume and sample.info data
...
write.csv(Photo, file="~/Documents/Projects/Thermal_Acclimation_2017/Output/Colony_Photo_Resp_Output/Photosynthesis.rates.T1.csv")
write.csv(Resp, file="~/Documents/Projects/Thermal_Acclimation_2017/Output/Colony_Photo_Resp_Output/Respiration.rates.T1.csv")

#calculate gross photosynthesis Pnet -- Rdark
resp.data <- merge(Photo[,c(1,10,19)],Resp[,c(1,19)], by="Fragment.ID")
...
colnames(resp.data)[3] <- "Pnet_umol.cm2.hr"
colnames(resp.data) [4] <- "Rdark_umol.cm2.hr"
...
#T2
#PHOTOSYNTHESIS
path.p<-"~/Documents/Projects/Thermal_Acclimation_2017/Data/Colony_Respirometry/T2" #the location of all your respirometry files
file.names<-list.files(path = path.p, pattern = "csv$") #list all csv file names in the folder
Photo.R <- data.frame(matrix(NA, nrow=length(file.names)*2, ncol=3)) #generate a 3 column dataframe with specific column names
colnames(Photo.R) <- c("Fragment.ID","Intercept", "umol.L.sec")
...
for(i in 1:length(file.names)) { # for every file in list start at the first and run this following function
  ...
  for(j in 1:length(lt.levs)){    
    ...
    #Save plot prior to and after data thinning to make sure thinning is not too extreme
    rename <- sub("_.*", "", file.names[i])
    pdf(paste0("~/Documents/Projects/Thermal_Acclimation_2017/Output/Colony_Photo_Resp_Output/T2",rename,"_",j,"thinning.pdf"))
    par(omi=rep(0.3, 4)) #set size of the outer margins in inches
    par(mfrow=c(1,2)) #set number of rows and columns in multi plot graphic
    plot(Value ~ sec, data=Photo.Data , xlab='Time (seconds)', ylab=substitute(' O'[2]~' (µmol/L)'), type='n', axes=FALSE) #plot data as a function of time
    usr  <-  par('usr')
    ...
    Regs  <-  rankLocReg(xall=Photo.Data $sec, yall=Photo.Data $Value, alpha=0.2, 
                         method="pc", verbose=TRUE) 
    pdf(paste0("~/Documents/Projects/Thermal_Acclimation_2017/Output/Colony_Photo_Resp_Output/T2",rename,"_",j,"regression.pdf"))
    plot(Regs)
    dev.off()
...
#T2 Results
#remove blanks
Photo2 <- subset(Photo2, Treatment!= "Blank")
Resp2 <- subset(Resp2, Treatment!= "Blank")

write.csv(Photo2, file="~/Documents/Projects/Thermal_Acclimation_2017/Output/Colony_Photo_Resp_Output/Photosynthesis.rates.T2.csv")
write.csv(Resp2, file="~/Documents/Projects/Thermal_Acclimation_2017/Output/Colony_Photo_Resp_Output/Respiration.rates.T2.csv")

#calculate gross photosynthesis Pnet -- Rdark
Photo2
resp.data <- merge(Photo2[,c(1,10,11,12,19)],Resp2[,c(1,19)],  by="Fragment.ID")
...
#write Results
write.csv(resp.data, file="../Data/AllRates.csv") # raw data
write.csv(AllMeans2, file="../Output/AllMeans.csv") # Mean data
...
#PLot the means
Fig.Pn <-  ggplot(AllMeans2, aes(x=Treatment, y=Pnet.mean,  color=Origin, group=Origin)) + #set up plot information
...
ylab(expression("Mean Larval Release per" ~ cm^{2}))

Fig.Pg <-  ggplot(AllMeans2, aes(x=Treatment, y=Pgross.mean,  color=Origin, group=Origin)) + #set up plot information
...
Fig.Rd <-  ggplot(AllMeans2, aes(x=Treatment, y=Rdark.mean,  color=Origin, group=Origin)) + #set up plot information
  geom_errorbar(aes(x=Treatment, ymax=Rdark.mean+Rdark.se, ymin=Rdark.mean-Rdark.se), colour="black", width=.1, position = pd) + #add standard error bars about the mean
  geom_line(position=pd, color="black")+
  xlab("Treatment") + #label x axis
  ylab(expression("Oxygen Flux " (mu~mol ~ cm^{-2} ~ h^{-1}))) + #label y axis
  ylim(-0.5, 1)+
...
Resp.Figs <- arrangeGrob ( Fig.Pn, Fig.Pg, Fig.Rd,ncol=3)
ggsave(file="~/Documents/Projects/Thermal_Acclimation_2017/Output/Colony_Photo_Resp_Output/T2_ColonyRespirometry.pdf", Resp.Figs, width = 11, height = 6, units = c("in"))
